...
from swissgeo_provider import (  # noqa: E402
    _apply_lang,
    _ensure_self_link,
    _get_lang_and_fmt,
    _local,
...
class TestGetLangAndFmt:
    ...
    def test_supported_lang_returned_as_is(self):
        set_request_params(lang="de", fmt=None)
        lang, fmt = _get_lang_and_fmt()
        assert lang == "de"

    @pytest.mark.parametrize("code", ["de", "fr", "it", "en"])
    def test_all_supported_langs(self, code):
    ...
    def test_locale_tag_stripped_to_primary(self):
        set_request_params(lang="de-CH", fmt=None)
        lang, _ = _get_lang_and_fmt()
        assert lang == "de"

    def test_underscore_locale_stripped(self):
        set_request_params(lang="fr_CH", fmt=None)
        lang, _ = _get_lang_and_fmt()
        assert lang == "fr"

    def test_fmt_propagated(self):
...
class TestApplyLang:
    def test_title_replaced_with_localised_variant(self):
        props = {"title": "Default", "title_de": "Deutsch"}
        _apply_lang(props, "de")
        assert props["title"] == "Deutsch"

    def test_description_replaced_with_localised_variant(self):
        props = {"description": "Default", "description_fr": "Français"}
        _apply_lang(props, "fr")
        assert props["description"] == "Français"

    def test_falls_back_to_original_when_localised_empty(self):
        props = {"title": "Default", "title_de": ""}
        _apply_lang(props, "de")
        assert props["title"] == "Default"

    def test_falls_back_to_original_when_localised_missing(self):
        props = {"title": "Default"}
        _apply_lang(props, "it")
        assert props["title"] == "Default"

    def test_per_lang_fields_stripped(self):
        props = {
            "title": "T",
            "title_de": "T-de",
            "title_fr": "T-fr",
    ...
    def test_non_lang_fields_untouched(self):
...
class TestEnsureSelfLink:
    ...
    def test_inserts_self_link_when_absent(self):
        links: list = []
    ...
    def test_does_not_duplicate_self_link(self):
        links = [{"rel": "self", "href": "http://example.com/existing"}]
        _ensure_self_link(links, "my-collection", "item-1")
        assert len(links) == 1

    def test_skips_when_item_id_empty(self):
        links: list = []
        _ensure_self_link(links, "my-collection", "")
        assert links == []

    def test_prepends_server_url_when_available(self):
    ...
    def test_self_link_type_is_geojson(self):
        links: list = []
...
class TestPatchLinks:
    ...
    def test_skips_link_with_empty_href(self):
        links = [{"href": ""}]
        _patch_links(links, "de", None)
        assert links[0]["href"] == ""

    def test_prepends_server_url_to_relative_link(self):
        set_request_params(lang=None, fmt=None, server_url="https://api.example.com")
